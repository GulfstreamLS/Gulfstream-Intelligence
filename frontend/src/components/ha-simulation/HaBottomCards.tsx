...
function Skeleton() {
  return (
    <div className="space-y-4 animate-pulse">
      {[...Array(4)].map((_, i) => <div key={i} className="h-8 bg-gs-border rounded-lg" />)}
    </div>
  );
}

function Empty({ text }: { text: string }) {
  return <p className="text-xs text-gs-muted font-bold text-center py-6">{text}</p>;
}
    ...
    <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
      <div className="bg-gs-card p-6 rounded-xl border border-gs-border shadow-sm">
        <div className="flex items-center gap-2 mb-6">
          ...
          <h3 className="font-bold text-gs-text">Key Concerns Identified</h3>
        </div>
          ...
          <div className="space-y-4">
            {concerns.map(c => (
              <div key={c.id} className="flex justify-between items-center gap-3">
                <p className="text-xs font-semibold text-gs-muted">{c.text}</p>
      ...
      <div className="bg-gs-card p-6 rounded-xl border border-gs-border shadow-sm">
        <div className="flex items-center gap-2 mb-6">
          ...
          <h3 className="font-bold text-gs-text">Likely Follow-up Questions</h3>
        </div>
          ...
          <div className="space-y-4">
            {followups.map(f => (
              <div key={f.id} className="flex justify-between items-center group cursor-pointer">
                <p className="text-xs font-semibold text-gs-muted group-hover:text-indigo-600 transition-colors">{f.text}</p>
                <ChevronRight size={14} className="text-gs-muted shrink-0 ml-2" />
              </div>
            ))}
          </div>
        )}
      ...
      <div className="bg-gs-card p-6 rounded-xl border border-gs-border shadow-sm">
        <div className="flex items-center gap-2 mb-6">
          ...
          <h3 className="font-bold text-gs-text">Recommended Actions</h3>
        </div>
          ...
          <div className="space-y-4">
            {actions.map(a => (
              <div key={a.id} className="flex justify-between items-center group cursor-pointer">
                <p className="text-xs font-semibold text-gs-muted group-hover:text-emerald-600 transition-colors">{a.text}</p>
                <ChevronRight size={14} className="text-gs-muted shrink-0 ml-2" />
              </div>
            ))}
          </div>
