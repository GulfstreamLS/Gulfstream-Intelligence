...
class ExportService:
    async def generate_pdf(self, analysis_data: Dict[str, Any], filename: str) -> str:
        ...
        auth_style = ParagraphStyle(
            'AuthStyle', parent=styles['Heading2'], fontSize=14, spaceBefore=15, spaceAfter=10, textColor=colors.HexColor("#4f46e5")
        )
        ...
        for auth, data in analysis_data.items():
            ...
            if data.get('gaps'):
                ...
                for gap in data['gaps']:
                    ...
                    elements.append(Paragraph(f"<i>Strategic Action:</i> {gap.get('recommended_action')}", styles['Normal']))
            ...
            outlook_text = (
                "Based on the analysis, it is recommended to prioritize the 'Critical' gaps before the next submission milestone. "
                "Ensure that all manufacturing process validation data is synchronized with the clinical study protocol. "
                "Regular audits of the stability data should be conducted to support the proposed shelf-life."
            )
    ...
    async def generate_docx(self, analysis_data: Dict[str, Any], filename: str) -> str:
        ...
        for auth, data in analysis_data.items():
            ...
            doc.add_paragraph(
                "Incorporate the missing criteria, specify precise storage temperatures, and ensure detailed stability data is added "
                "to improve the submission's readiness for regulatory review."
            )
        
        buffer = io.BytesIO()
        doc.save(buffer)
        content = buffer.getvalue()
        url = await storage_service.upload_file(content, f"Report_{filename}.docx", "application/vnd.openxmlformats-officedocument.wordprocessingml.document")
        return url
    ...
    async def generate_pptx(self, analysis_data: Dict[str, Any], filename: str) -> str:
        ...
        prs.save(buffer)
        content = buffer.getvalue()
        url = await storage_service.upload_file(content, f"Report_{filename}.pptx", "application/vnd.openxmlformats-officedocument.presentationml.presentation")
        return url
