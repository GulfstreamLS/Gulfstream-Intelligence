...
class VectorService:
    ...
    def embeddings(self):
        if self._embeddings is None:
            ...
            self._embeddings = OpenAIEmbeddings(api_key=settings.OPENAI_API_KEY, model="text-embedding-3-small")
        return self._embeddings

    async def add_regulatory_content(
        self, db: AsyncSession, authority: str, title: str, content: str, source_url: str = None, metadata: dict = None
    ) -> None:
    ...
    async def search_regulatory_context(
        self, db: AsyncSession, query: str, authority: str | list[str] = None, limit: int = 5
