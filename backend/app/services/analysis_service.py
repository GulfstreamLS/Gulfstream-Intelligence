...
from app.models.regulatory import Action, AnalysisDocument, Gap, Insight
...
class AnalysisService:
    ...
    def llm(self):
        if self._llm is None:
            # We default to gpt-4o for heavy analysis as it currently handles structured output best
            # for the regulatory schema. In the future, this can be switched to Claude 3.5 Sonnet.
            model_name = getattr(settings, "ANALYSIS_MODEL", "gpt-4o")
    ...
    async def analyze_document_multi(
        ...
        for auth in authorities:
            ...
            context_sources = await vector_service.search_regulatory_context(db, search_query, authority=auth, limit=10)
            context_text = "\n\n".join(
                [f"SOURCE: {s.title} ({s.authority})\nCONTENT: {s.content}" for s in context_sources]
            )
